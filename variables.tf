...
variable "vcn_subnets" {
  ...
  validation {
    condition     = length(var.example_map) == 0 || length(distinct(values([for v in var.example_map : v.dns_label if v.dns_label != null]))) == length([for v in var.example_map : v.dns_label if v.dns_label != null])
